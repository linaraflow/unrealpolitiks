...
var menu_to_close: Array = ["CountryMenu", "ProvinceMenu", "DivisionMenu", "TopMenu"]
...
func _on_send_demands_pressed() -> void:
    ...
    for p_id in _claimed:
        ProvinceRegistry.province_data[str(p_id)]["against_occupation"] = ""
        ProvinceRegistry.province_occupants.erase(p_id)
        # ВАЖНО: официальная аннексия по миру делает нас НАСТОЯЩИМ ("core") владельцем.
        # Без этого core_owner навсегда останется прежней страной (например, Iran),
        # и провинция будет некорректно "возвращаться" ей в будущих войнах/мирах.
