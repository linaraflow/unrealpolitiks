...
func _ready() -> void:
    ...
    # Заполняем список сложности и выставляем Standard по умолчанию.
    difficulty_option.clear()
    difficulty_option.add_item("Easy", GameSettings.Difficulty.EASY)
    difficulty_option.add_item("Standard", GameSettings.Difficulty.STANDARD)
    difficulty_option.add_item("Hard", GameSettings.Difficulty.HARD)
    difficulty_option.add_item("Hardcore", GameSettings.Difficulty.HARDCORE)
