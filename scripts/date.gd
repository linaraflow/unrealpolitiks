...
func _ready() -> void:
	...
	for button in [$PlusSpeedButton, $MinusSpeedButton, $PauseButton]:
...
func _on_clock_day_passed(_date: Dictionary) -> void:
	$DateLabel.text = GameClock.get_date_string()
...
func _on_clock_pause_changed(is_paused: bool) -> void:
	...
	$PauseButton.icon = load(icon_path)

func _update_speed_label(_new_speed: int = 0) -> void:
	$SpeedPanel/SpeedLabel.text = "x" + str(GameClock.SPEEDS[GameClock.speed_index])
